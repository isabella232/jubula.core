...
\makeatletter
\section{Release Notes for \jb{} 4.3.01105}
\makeatother
\subsection{Information}
This release is a patch to fix a problem that occurred in the \bxname{unbound modules concrete} whereby the \bxname{tree} component had a false component type. The database and license are the same as in version 4.3.01105. 
...
\begin{itemize}
\item The Mylyn plugin is now available for use within \jb{}. 
\item You can connect to repositories (e.g. bug-tracking systems) to work on testing tasks directly in \jb{}.
...
\begin{itemize}
\item \jb{} has a new editor for managing data centrally for a \gdproject{}
...
\begin{itemize}
\item \jb{} has new options to search for items and test data within a \gdproject{}.
...
\item [Tasks]{Tasks from repositories connected to \jb{}.}
\end{description}
...
\begin{itemize}
\item \jb{} now allows the definition of manual \gdcases{}
...
\begin{itemize}
\item Custom renderers in Swing \gdauts{} can now be tested with \jb{} as long as the renderer implements either getText() or getTestableText(). 
...
\begin{itemize}
\item We recommend setting up a new workspace for version 4.3 of \jb{}. 
\end{itemize}
...
\end{itemize}
\makeatletter
\section{Release Notes for \jb{} 4.2.01053}
\makeatother
\subsection{New features and developments}

\textbf{Eclipse 3.6 supported}
\begin{itemize}
\item \jb{} is compatible with Eclipse applications that are written in 3.6
\end{itemize}

\textbf{Viewing test results in \jb{}}
\begin{itemize}
...
\item This lets you review tests that ran overnight directly in the \jb{} client. 
\end{itemize}

\textbf{Automatic \gddb{} migration}
\begin{itemize}
\item \jb{} now contains a \gddb{} migration wizard.
\item When a connection to a \gddb{} is made in a new version, \jb{} automatically recognizes whether the \gddb{} version is incompatile. 
\item If so, you can start the migration wizard. You specify which \gdprojects{} \jb{} should import into the new \gddb{} (the \gdprojects must have been exported from the previous version).
...
\begin{itemize}
\item \gdauts{} that have been started using the \bxname{gdrun} command can now be automatically defined in the \jb{} client.
\item You can select the option to create an \gdaut{} definition from the running \gdauts{} view for any \gdaut{} that is currently unknown to \jb{}.
\end{itemize}
...
\begin{itemize}
\item The XML and HTML reports that can be generated by \jb{} (configurable in the preferences) are scheduled for removal in the next versions. 
\item There will be an alternative feature to create a HTML report from the test result report in \jb{}, as long as the test details are still in the \gddb{}.
...
\makeatletter
\section{Release Notes for \jb{} 4.1.01039}
\makeatother
\subsection{New features and developments}
...
\begin{itemize}
\item You can now define whether or not \jb{} should automatically take a screenshot when an error occurs. 
\item Screenshots taken in this manner are saved into the \gddb{} and can be viewed in the \gdimgview{} in \jb{} by clicking on the failed \gdstep{}. 
...
\begin{itemize}
\item The functionality to run \jb{} as an Eclipse plugin has been removed.
\end{itemize}
...
\item There are also two new options: MySQl and Postgresql. These are shown as unsupported: we have tested that \jb{} can connect to them and open \gdprojects{}, but do not let our full tests run on them at this time. 
\end{itemize}
...
\makeatletter
\section{Release Notes for \jb{} 4.0.01205}
\makeatother
\subsection{New features and developments}
...
\textbf{New handling for starting \gdauts{} from within \jb{}}
\begin{itemize}
\item It is now possible to start multiple \gdauts{} from \jb{}. 
...
\item Some buttons that were previously on the toolbar have been moved to the \gdtestsuitebrowser{} and the new \jb{}unautview{}. 
\end{itemize}

\textbf{New gdrun option to start \gdauts{} independently of \jb{}}
\begin{itemize}
\item \gdauts{} can now be started in one of two ways. 
\item They can be started as in previous versions via an \gdaut{} configuration. There is also a new option to start an \gdaut{} without an \gdaut{} configiration, independently of \jb{}. 
...
\end{itemize}
\textbf{New \jb{} reporting perspective}
\begin{itemize}
\item \jb{} has a new reporting perspective. After every test run, the test results are now stored in the \gddb{}. Both detailed results and a test result summary are saved.
...
\begin{itemize}
\item From the \gdtestsummaryview{}, you can create \jb{} BIRT reports of your test runs. 
...
\item Pure SWT \gdauts{} cannot be started by \jb{} on Mac systems. RCP \gdauts{}, however, can be. 
\end{itemize}
...
\subsection{Information}
\textbf{Altered XML Scheme for extensions to \jb{}}
\begin{itemize}
\item We have made a small change to the XML scheme for \jb{} extensions. 
...
\begin{itemize}
\item To reflect the change in behavior of the \jb{} server component, we have renamed the AUT Starter to AUT Agent.
...
\textbf{New licenses}
\begin{itemize}
\item \jb{} 4.0 requires new licenses. You can request your new license from the support team.
\end{itemize}
...
\item Full details about the new support service are in the license agreement and in the \jb{} shop.
\end{itemize}

\textbf{Alternative classloading}
\begin{itemize}
\item We have made some changes to the classloading for \gdauts{} started with \jb{}.
...
\begin{itemize}
\item \jb{} cannot currently stop an \gdaut{} during a test. 
...
\clearpage
\makeatletter
\section{Release Notes for \jb{} 3.2.02004}
\makeatother
...
\begin{itemize}
\item The software license that you agree to when you download or install \jb{} has been updated.
...
\makeatletter
\section{Release Notes for \jb{} 3.2.01081}
\makeatother

\subsection{New features and developments}
\textbf{New actions}
\begin{itemize}
\item This version of \jb{} contains a variety of new actions to:
\begin{itemize}
...
\begin{itemize}
\item \jb{} now contains a new option in the object mapping preferences to refresh components in a web application. This can be used during object mapping to recognize newly added components since the last refresh.
...
\begin{itemize}
\item There is a new \gddb{} configuration tool to select, add and configure \gddb{}s for use with \jb{}. More information on this is available in the Installation Manual.
\item It is now possible to switch between \gddb{}s or \gddb{} users in the \jb{} client. This saves having to restart the client to switch the \gddb{}. 
\end{itemize}

\textbf{Windows 7 supported}
\begin{itemize}
\item \jb{} 3.2 has been tested with Windows 7 as well as XP and Vista. 
\end{itemize}
...
\item Run these tools as an administrator to be able to configure your \jb{} installation. 
\end{itemize}
...
\section{Release Notes for \jb{} 3.1.2003}
...
\makeatletter
\section{Release Notes for \jb{} 3.1.01019}
\makeatother
...
\begin{itemize}
\item \jb{} is now compatible with Eclipse 3.5 and can test applications written in 3.5. 
...
\item \jb{} can now select and check the selection of checkboxes on SWT Trees. 
\end{itemize}
...
\begin{itemize}
\item We have added control decoration to various components in the user interface to help new users understand some of the more advanced options with \jb{}. 
\end{itemize}
...
\begin{itemize}
\item In the \jb{} preferences, you can choose to hide the information behind \gdstep{} names.% \bxextref{\gduserman}{user,gdprefs}. 
...
\begin{itemize}
\item When no \gdproject{} is open, connecting to the \gdserver{} and disconnecting again results in no longer being able to connect to the \gdserver{} until \jb{} is restarted. 
\end{itemize}
...
\begin{itemize}
\item These are not supported by \jb{}. 
\end{itemize}
...
%% \clearpage
%% \makeatletter
%% \section{Release Notes for \jb{} 3.0.2001}
%% \subsection{New features and developments}
%% \makeatother
%% \textbf{Model-based testing}
%% \begin{itemize}
%% \item \jb{} has a new modeling perspective. 
...
%% \item You can now record actions directly from a running \gdaut{}. The actions are stored as \jb{} \gdsteps{} and can be easily combined with each other to make reusable modules. 
...
%% \begin{itemize}
%% \item The \jb{} license mechanism has been changed. 
%% \item The main effect of this is that test executions started from the command line client now no longer need a full \jb{} license. 
...
%% \item Testers working with the \jb{} client (i.e. creating tests) still need \bxname{specification licenses}, which correspond to the old full licenses. 
...
%% \begin{itemize}
%% \item It is now possible to import and export \gdprojects{} fromt he \gddb{} without starting the \jb{} client. 
...
%% \begin{description}
%% \item [The tree view]{ is the \gdomeditor{} known from other versions of \jb{}.}
...
%% \item If you save Excel data files and HTML result reports to your workspace, you will be able to view (and edit) them directly in \jb{} using the in-place editor. 
%% \end{itemize}
...
%% \begin{itemize}
%% \item We have added a new chapter to the documentation on best practices for working with \jb{}.  
%% \end{itemize}
...
%% \begin{itemize}
%% \item In \jb{}, you can select multiple items from a list by comma-separating them. 
...
%% \clearpage
%% \section{Release Notes for \jb{} Version 2.2.01013}
%% \subsection{New features and developments}

%% \textbf{Cheat sheets}
%% \begin{itemize}
%% \item The tutorials in the user manual have been replaced with Cheat Sheets.
%% \item The Cheat Sheets can be opened from the Help menu in the \jb{} client. 
%% \item We recommend that new users of \jb{} start with these Cheat Sheets. 
%% \end{itemize}
...
%% \textbf{\jb{} variables for use in \gdsuites{}}
%% \begin{itemize}
%% \item \jb{} now contains pre-defined test execution variables, which can be used in tests. 
...
%% \begin{itemize}
%% \item \jb{} has migrated to Eclipse 3.4. 
%% \item The \jb{} plugin can be run in Eclipse 3.4 or Eclipse 3.3. 
%% \end{itemize}

%% \textbf{Support emails}
%% \begin{itemize}
%% \item In the Help menu in \jb{}, emails to our support team can now be generated. 
...
%% \textbf{Welcome page}
%% \begin{itemize}
%% \item \jb{} now has a welcome page containing information on the example projects, the cheat sheets as well as general information about the program. 
%% \item The welcome screen can be viewed using the Help menu. 
%% \end{itemize}

%% \textbf{Updated example \gdprojects{}}
%% \begin{itemize}
%% \item The example \gdprojects{} have been updated to reflect the best way of working with \jb{}. 
%% \item They are described in chapter 2 of the user manual. 
%% \item You can import the sample \gdprojects{} from the \bxname{aut} directory in the \jb{} installtion. 
%% \end{itemize}
...
%% \begin{itemize}
%% \item There is a known issue with the opening of context menus with \jb{} on Unix systems. 
...
%% \section{Release Notes for \jb{} Version 2.1.01018}


%% \subsection{New features and developments}
%% \textbf{Eclipse 3.3 support}
%% \begin{itemize}
%% \item \jb{} now supports Eclipse 3.3. 
%% \item The \jb{} plugin can be used with the Eclipse 3.2 or 3.3 releases. 
%% \item Java 1.5 or higher is required for the plugin version.
%% \end{itemize}

%% \textbf{Drag and drop actions}
%% \begin{itemize}
%% \item \jb{} now supports drag and drop actions on various components. 
...
%% \begin{itemize}
%% \item The \jb{}-server has been renamed to the \gdserver{} to make its role clearer. 
%% \end{itemize}

%% \textbf{New \gdcase library}
%% \begin{itemize}
%% \item We have improved and expanded the \gdcase{} library installed with \jb{}. 
...
%% \item For the internal \gddb{}, no configuration or login to the \gddb{} is necessary. This makes running \jb{} easier in the evaluation stages. 
%% \item For the multi-user \gddb{}, the \jb{} database scheme is automatically created for the user account at the first login to the \gddb{}. 
%% \end{itemize}

%% %\subsection{Bug fixes and workarounds}
%% \subsection{Other Information}
%% \begin{description}
%% \item [Configuring an SWT \gdaut{}:]{There are two different ways of starting SWT \gdauts{}. At the moment, \jb{} cannot start SWT \gdauts{} via \bxname{java.library.path}. Please use \bxname{-Djava.ext.dirs} as a JRE argument in the \gdaut{} configuration to start your SWT \gdaut{}. }
%% \item [Speed in some configurations:]{In some configurations of Linux and RemoteX, Eclipse runs extremely slowly. As \jb{} is based on Eclipse, on particular configurations it may also run slowly. }
%% \item []{}
%% \end{description}
%% \section{Release Notes for \jb{}Version 2.0.01052}
...
%% \begin{itemize}
%% \item The range of toolkits testable with \jb{} has been extended. 
%% \item \jb{} can now test Swing, SWT, RCP and Web (HTML) \gdauts{}. 
%% \item For RCP and Web \gdauts{}, certain steps are necessary to configure either \jb{} or the \gdaut{} before testing. Refer to the installation manual and the user manual for more details. 
%% \end{itemize}
...
%% \textbf{UTF-16}
%% \begin{itemize}
%% \item \jb{} now supports UTF-16 encoding. 
%% \item Refer to the installation manual for recommendations on a \gddb{} for UTF-16.
%% \end{itemize}
%% \textbf{Show log}
%% \begin{itemize}
%% \item If you have configured \jb{} to create log files, you can show these in the client via the help menu. 
%% \end{itemize}
...
%% \item[Component Hierarchy]{The component hierarchy has been modified since the previous release. Where possible, \jb{}will perform all necessary \gdproject modificatations during the import/conversion process. This will result in, for example, all components that were previously Clickable Components being converted to Graphics Components, as the type Clickable Component no longer exists. Please refer to the Reference Manual for the updated component hierarchy.}
...
%% \item [Extending \jb{}]{There are new procedures for extending components and creating your own new components. Refer to the extension manual for details. }
...
%% \section{Release Notes for \jb{}Version 1.2.02026}
%% \subsection{New Features and Developments}
...
%% \section{Release Notes for \jb{}Version 1.2.01031}
%% \subsection{Important advice}
%% It is, as always, strongly recommended that you back up all projects (by exporting to XML) as well as any Implementation Classes you may have (both source and guidancerConfig.xml) before upgrading to this version of \gd.
%% \subsection{New Features and Developments}
%% \textbf{Simplified Installation}
%% \begin{itemize}
%% \item Tools such as the \jb{} Configuration tool and the License Manager can now be started directly from the installer.
%% \end{itemize}
...
%% \item Both Client and Server (when connected to a GDServer) logs are available from the \jb{}\bxname{Help} menu.
%% \end{itemize}
%% \textbf{Eclipse 3.2}
%% \begin{itemize}
%% \item \jb{}now uses and supports Eclipse 3.2.
%% \item This plugin version does not support Eclipse 3.1 or lower.
%% \end{itemize}
%% \textbf{Testcase Template and Guide}
%% \begin{itemize}
%% \item Reusable Testcases and a usage Guide are now included in the \jb{}installation.
%% \item The Testcase Template Guide can be found with the rest of the \jb{}documentation.
...
%% \item See the \bxname{Shortcuts} section of the \jb{}Reference Manual.
%% \end{itemize}
%% \textbf{Improved Problem View}
%% \begin{itemize}
%% \item The \jb{}Problem View has additional support for identification and resolution of common problems, such as connection to a GDServer for which no AUT Configuration is currently defined.
%% \end{itemize}

%% \subsection{Other Information}
%% \begin{description}
%% \item[Implementation Classes and Internationalization]{Internationalization strings for \jb{}have been separated into compSystemStrings.properties (which contains all strings related to the component system) and guidancerStrings.properties (which contains strings specifically related to \gd). This separation will require users with their own Implementation Classes to move all internationalization strings for these classes to \emph{compSystemStrings.properties}.}
%% \end{description}
%% \begin{description}
%% \item["Could not restore workbench layout." Error]{You may encounter this error if you use the same workspace for \jb{}1.2 as you did for \jb{}1.1. If you receive this error message, click \bxshell{OK} to close the message window, then click \bxmenu{Window}{Reset Perspective}{} to solve the problem.}
%% \end{description}
%% \begin{description}
%% \item[Separation of User Manual and Reference Manual]{The \bxname{References} section of the \jb{}User Manual has been removed and is now available as a separate document: The \jb{}Reference Manual.}
%% \end{description}
%% \begin{description}
...
%% \section{Release Notes for \jb{}Version 1.1.02026}
%% \subsection{New Features and Developments}
%% \textbf{Vastly Improved Performance}
%% \begin{itemize}
%% \item Amount of time required for various operations (such as opening and saving of \gdcases) has been greatly reduced.
...
%% \section{Release Notes for \jb{}Version 1.1.02016}
%% \subsection{New Features and Developments}
...
%% \item See the \emph{References -> Actions And Parameters} section of the \jb{}Manual for specific information.
%% \end{itemize}
...
%% \item See the \emph{Tasks -> Testing without the user interface} section of the \jb{}Manual for further information.
%% \end{itemize}

%% \textbf{Supported Java versions}
%% \begin{itemize}
%% \item \jb{}now officially supports strictly \gdaut{}s compiled with Java 1.4.1 or higher.
%% \item \jb{}is also not officially tested with Java 1.6.
%% \end{itemize}
...
%% \section{Release Notes for \jb{}Version 1.1.02002}
%% \subsection{Bug fixes and workarounds}
...
%% \section{Release Notes for \jb{} Version 1.1}

%% \subsection{Important advice}
%% If you have previously used \jb{} and wish to continue using your \gdprojects{} and database, please follow these instructions to ensure that your \gdprojects{} can be used in the new version. 
%% \begin{enumerate}
%% \item In your old \jb{} client, export your \gdprojects{} out of the database. 
...
%% \end{enumerate}
%% These steps are necessary due to changes in the \jb{} client which will not otherwise be compatible between versions. 
...
%% \jb{} has undergone some changes to the user interface. The icons have been improved. For information on any new icons, and to see what they mean, consult the \bxcaption{Icons} section in the References chapter of the handbook.

%% \textbf{Changes to preference pages}

%% \begin{itemize}
%% \item The preferences pages have been extended to include more preferences. 
%% \item See the \jb{} handbook and the preference pages themselves for more information. 
%% \end{itemize}
...
%% \item You can also set environment variables to be used in \jb{}. 
...
%% \clearpage 
%% \section{Release Notes for \jb{} Version 1.0.64}
%% \begin{itemize}
...
%% \begin{itemize}
%% \item Multilingual testing is now possible with \jb{}.
...
%% \begin{itemize}
%% \item \jb{} now allows you to execute tests without the user interface.
...
%% \begin{itemize}
%% \item Text fields in \jb{} (e.g. in the \gdpropview{}) now offer help when you are entering values.
...
%% \begin{itemize}
%% \item \jb{} now supports more actions and offers more parameters. 
...
%% \begin{itemize}
%% \item You can now search \jb{} browsers and editors via:\\
%% \bxmenu{Edit}{Search}{}.
%% \end{itemize}
...
%% \textbf{Status Bar}
%% \begin{itemize}
%% \item The status bar is now at the bottom right of the \jb{} client.  
%% \end{itemize}

%% \clearpage
%% \section{Release Notes for \jb{}Version 1.0.03}
%% \subsection{New Features and Developments}
%% \begin{description}
...
%% \subsection{Other Information}
%% \begin{description}
%% \item[Java Policies]{If the \gdaut uses a security manager (or, in some cases, a Java Policy), please be aware that the \jb{}service component requires permission to create and use ClassLoader instances. Most policies will not permit this action for security reasons. The service component uses the ClassLoader to separate the program code used to inspect the \gdaut from the \gdaut itself as much as possible. 

%% To grant the necessary permission, please use the following policy snippet (please use a personal installation directory instead of  the \jb{}installation directory). Please also be aware that additional line breaks have been inserted in the following snippet. 
...
%% \section{Release Notes for \jb{}Version 1.0.02}

%% \subsection{New Features and Developments}
%% \begin{description}
%% \item[Mac OS and Unix Platforms]{\jb{}on Mac OS and Unix has now been tested; the installation for these operating systems is included in this release. } 
%% \item [Eclipse Plugin License Error fixed]{In Version 1.0, starting the \jb{}
...
%% \item [Opening Context-Sensitive Menus using the Keyboard]{As an alternative to performing a right-click, \jb{}supports standard keyboard-shortcuts to open context-sensitive menus, e.g. \bxkey{Shift+F10} in Windows, and \bxkey{Ctrl+Space} in Mac OS.  }
...
%% \subsection{New Features and Developments}
%% \begin{description}
%% \item[AUT Activation Option]{Some \gdauts may need activating before testing can begin, especially those running on the same machine as the \jb{}client. \jb{}
...
%% \subsection{Functional Issues}
%% \begin{description}
%% \item[Mac OS and Unix Support]{As support for \jb{} on Mac OS and Unix operating systems is still under
