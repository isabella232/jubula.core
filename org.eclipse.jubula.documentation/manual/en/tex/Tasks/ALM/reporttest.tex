...
\item In the \bxname{ALM Report Pending} column, you will see whether there are any reports pending. This is based on various factors:
\begin{enumerate}
\item ALM Reporting must have been configured for the \gdproject{}. If no ALM is configured for the \gdproject{}, then the status in the \bxname{ALM Report Pending} column will be \bxname{false}.
\item The test run must have been designated as relevant. Non-relevant tests show the status \bxname{false} in the \bxname{ALM Report Pending} column.
\end{enumerate}
...
\item When you select this option, the reporting will be triggered as described above. If the reporting is successful, the status in the \bxname{ALM Report Pending} column changes from \bxname{true} to \bxname{false}. If an error occurs, then the status remains as \bxname{true}. 
\bxtipp{If you do not want to report the test results to an ALM tool, you can switch the status of the \bxname{ALM Report Pending} column from \bxname{true} to \bxname{false} by selecting the option to cancel pending reports from the context menu. Toggling the relevance of the test run to \bxname{false} will also set the  \bxname{ALM Report Pending status to \bxname{false}, but this will also mean that the test results are not exported and not considered for BIRT reports \bxpref{testresprefs}.}}
\end{enumerate}
