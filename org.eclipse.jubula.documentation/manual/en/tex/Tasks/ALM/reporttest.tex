...
\item If there are comments to be added, \ite{} connects to the repository and writes one comment per task, as defined in the \gdproject{} properties. If multiple items reference the same task ID, if an item with a task ID is executed multiple times, or if a \gdjob{} has a task ID, then each test status and result link is written, but all in one comment. 
