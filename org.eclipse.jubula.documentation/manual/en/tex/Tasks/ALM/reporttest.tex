...
\bxtipp{If you do not want to report the test results to an ALM tool, you can switch the status of the \bxname{ALM Report Pending} column from \bxname{true} to \bxname{false} by selecting the option to remove pending reports from the context menu. Toggling the relevance of the test run to \bxname{false} will also set the  \bxname{ALM Report Pending status to \bxname{false}, but this will also mean that the test results are not exported and not considered for BIRt reports \bxpref{testresprefs}.}}
\end{enumerate}
