...
\bxtipp{If you want to start your \gdaut{} yourself, and have \app{} connect to it, then use the \bxname{gdrun} command to start the \gdaut{} \bxpref{gdrun}. In this case, you do not need to create an \gdaut{} configuration.}
...
%\item you can also enter a \bxname{component ID tag}. If you have used a specific tag to name components in your application, enter the tag here. \app{} will then use this information instead of the \bxname{name} attribute in the object recognition. 
\end{enumerate}
...
\item You can optionally create a working/base directory to store files in. The \gdagent directory of the \app{} installation is selected as default. To create a base directory elsewhere, browse to and select the location. 

The base directory is the working directory for any classpaths, classnames, JAR files and JRE binaries. If you create a base directory, you can enter the paths to these items using a relative path, with the base directory as the root. For more information on relative paths, read the section in the reference manual \bxextref{\app{}efman}{ref,relativepath}. 
...
The Java version used must be 1.4 or later. For more information on adding a new JRE executable for this \gdagent, see the later section \bxpref{serverConfigPrefPage}. 
...
\app{} supports English (US) and German (DE) keyboard layouts out-of-the box. If you want to use a different keyboard layout, see the reference manual for information on creating keyboard layouts \bxextref{\app{}efman}{ref,keyboardlayout}. 
\end{enumerate}
