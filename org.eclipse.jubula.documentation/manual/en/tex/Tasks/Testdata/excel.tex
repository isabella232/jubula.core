...
\bxwarn{The addition of data via Excel is discouraged, because any problems with the data can only be found once the test is running. We recommend using the central data sets in \app{} to manage data within the \gdproject{} \bxpref{TasksCentralData}.}
\begin{enumerate}
...
\bxtipp{The Excel file must be configured in a specific way in order for \app{} to read it \bxpref{TasksConfigureExcel}}.
