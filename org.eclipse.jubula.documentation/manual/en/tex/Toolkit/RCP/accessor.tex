...
\begin{enumerate}
\item Locate the \app{} installation directory, and open the \bxname{development} folder.
\item Extract the content of the \bxname{rcp-support.zip} folder into the \bxname{plugins} directory for your RCP \gdaut{}.
\item RCP applications generally have a configuration/config.ini file which contains the parameter \bxname{osgi.bundles}. This parameter needs to be modified to allow the RCP remote control plugin to load on \gdaut{} startup. You must add  \bxname{org.eclipse.jubula.rc.rcp@start} to the end of the \bxname{osgi.bundles} parameter.
