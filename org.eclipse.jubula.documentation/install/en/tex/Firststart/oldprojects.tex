...
%\bxtipp{Please be aware that we can only guarantee that \gdprojects{} from the previous version can be successfully imported into a new version. \gdprojects{} from older versions of \app{} may not be imported directly into the newest version.}
...
\item If there has been a license change for \app{}: Uninstall and reinstall the license server. Make sure you do the reinstall on the same computer and that you start the license server on the same port as previously. Customers with maintenance packages can request a new license from the support team.
...
%\item Then import your other \gdprojects{} into the new \app{} \gddb{}.
\end{enumerate}

\section{Updating to the new version of the unbound modules}
We recommend that you always update the version of the unbound modules \gdprojects{} to the new versions installed with \app{}. You can do this via the \gdproject{} properties in the \app{} client. This is described in the user manual in the \bxname{Tasks} section under \bxname{\gdprojects{}}. 
