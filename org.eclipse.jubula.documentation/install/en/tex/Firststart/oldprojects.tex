If you have an older version of \gd{} installed, we recommend that you follow these steps:

%\bxtipp{Please be aware that we can only guarantee that \gdprojects{} from the previous version can be successfully imported into a new version. \gdprojects{} from older versions of \gd{} may not be imported directly into the newest version.}

\begin{enumerate}
\item From the old version of \gd{}, export and backup all the \gdprojects{} from the database. 
...
\item Back up any extensions you have made to \gd{}: any customized plugins and implementation classes you have written.  
...
\item Uninstall the old version of \gd{}. 
\item If there has been a license change for \gd{}: Uninstall and reinstall the license server. Make sure you do the reinstall on the same computer and that you start the license server on the same port as previously. Customers with maintenance packages can request a new license from the support team.
...
%\item Clear (empty) the \gddb{} schema for all necessary \gd{} users. 
\item Install the new version of \gd{}.
...
%\item Then import your other \gdprojects{} into the new \gd{} \gddb{}.
\end{enumerate}

\section{Updating to the new version of the unbound modules}
We recommend that you always update the version of the unbound modules \gdprojects{} to the new versions installed with \gd{}. You can do this via the \gdproject{} properties in the \gd{} client. This is described in the user manual in the \bxname{Tasks} section under \bxname{\gdprojects{}}. 
