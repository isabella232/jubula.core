...
\begin{description}
\item [\app{} client (standalone):]{JRE version 1.5 minimum}
...
For the \app{} client and the license administrator, you will need a 32-bit Java version. The operating system you use can be 32 or 64 bit, however. 
...
%% \item vs\_piaredist.exe (available on the \app{} website)
%% \end{itemize}

%% If you do not have these installed, you will not be able to run web tests. 

%% There is an example \gdproject{} in the \app{} installation called \bxname{WebTest}. It can be found under \bxname{guidancer/examples/projects}.  Try running this test on the \app{} test site (which is the \gdaut{} for the test). If the test runs, you have everything installed that you need. If not, check that you have the .NET and J\# packages installed in the correct version. 
...
\begin{enumerate}
\item From \app{} to the \gdagent{}: this port is defined in the \app{}  preferences.
\item From the \gdaut{} to \app{}: this port is dynamically chosen, and cannot currently be defined. Therefore, any ports available on your test machine must also be open on the machine from which the test is being run.
\end{enumerate}

 If opening all ports in this way is not an option, we recommend using the \app{} command line client on the test machine to run the tests, so that all communication is done locally. 
...
%The \app{} plugin can run in Eclipse version Ganymede (3.4). % and Galileo (3.5).
